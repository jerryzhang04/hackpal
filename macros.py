...
# def generate_prompt_pre(vars: Dict[str, Any]) -> str:
#     prompt_parts = []
#     available_states_pre = ['get_techstack_and_idea', 'another_state', 'yet_another_state']  # Add your available states here
#
#     if 'TRACK' not in vars:
#         prompt_parts.append('"TRACK": "Interested in healthcare, social good, finance, etc."')
#     if 'TECH_STACK' not in vars:
#         prompt_parts.append('"TECH_STACK": "Planning to use Python, JavaScript, Java, etc."')
#
#     prompt_parts.append(f'"NEXT_STATE": {"{" + ", ".join(f"{state}" for state in available_states_pre) + "}"}')
#
#     prompt = f'Please provide the missing information and choose only ONE next logically best state from the given options. ' \
#              f'You may ONLY choose from the given options. If no state seems best, summarize and reiterate your idea. ' \
#              f'IF ALL INFORMATION IS NOT COLLECTED, UNDER NO CIRCUMSTANCES SHOULD YOU GO TO THE SUMMARY STATE. ' \
